# 🔄 State Pattern Study Guide (状态模式学习指南)

> **Status**: 📚 Study Document  
> **Pattern Type**: Behavioral Design Pattern  
> **Primary Application**: Order Lifecycle Management (订单生命周期管理)  
> **Related Domain**: [04-ORDER-PROCESSING.md](../core-domains/04-ORDER-PROCESSING.md)

---

## 📖 Table of Contents

1. [Pattern Overview](#-pattern-overview)
2. [Problem It Solves](#-problem-it-solves)
3. [Pattern Structure](#-pattern-structure)
4. [Logistics Application](#-logistics-application)
5. [State vs Strategy Pattern](#-state-vs-strategy-pattern)
6. [Implementation Variations](#-implementation-variations)
7. [Anti-Patterns to Avoid](#-anti-patterns-to-avoid)
8. [Advanced Topics](#-advanced-topics)
9. [Chinese Tech References](#-chinese-tech-references)
10. [Self-Assessment](#-self-assessment)

---

## 🎯 Pattern Overview

### Definition (定义)

> **State Pattern** allows an object to alter its behavior when its internal state changes. The object will appear to change its class.
>
> **状态模式**允许对象在内部状态改变时改变它的行为。对象看起来好像修改了它的类。

### Visual Metaphor: Traffic Light (红绿灯比喻)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         THE TRAFFIC LIGHT ANALOGY                           │
│                         红绿灯的比喻                                         │
├─────────────────────────────────────────────────────────────────────────────┤
│  A traffic light behaves differently based on its current state:            │
│  红绿灯根据当前状态有不同的行为                                                │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────┐        │
│  │                                                                 │        │
│  │       🔴 RED STATE                                              │       │
│  │       ─────────────                                             │        │
│  │       • Vehicles STOP (车辆停止)                                 │        │
│  │       • Pedestrians can CROSS (行人可通行)                       │        │
│  │       • Duration: 30 seconds                                    │        │
│  │       • Next state: GREEN                                       │        │
│  │                                                                 │        │
│  │              │ timer expires                                    │        │
│  │              ▼                                                  │        │
│  │                                                                 │        │
│  │       🟢 GREEN STATE                                            │        │
│  │       ─────────────                                             │        │
│  │       • Vehicles GO (车辆通行)                                   │        │
│  │       • Pedestrians WAIT (行人等待)                              │        │
│  │       • Duration: 45 seconds                                    │        │
│  │       • Next state: YELLOW                                      │        │
│  │                                                                 │        │
│  │              │ timer expires                                    │        │
│  │              ▼                                                  │        │
│  │                                                                 │        │
│  │       🟡 YELLOW STATE                                           │        │
│  │       ─────────────                                             │        │
│  │       • Vehicles SLOW DOWN (车辆减速)                            │        │
│  │       • Pedestrians PREPARE (行人准备)                           │        │
│  │       • Duration: 5 seconds                                     │        │
│  │       • Next state: RED                                         │        │
│  │                                                                 │        │
│  └─────────────────────────────────────────────────────────────────┘        │
│                                                                             │
│  KEY INSIGHT:                                                               │
│  • Same traffic light, different behaviors                                  │
│  • Each state knows what it can do and what comes next                      │
│  • Invalid transitions prevented (can't go RED → YELLOW directly)           │
│                                                                             │
│  In Order Processing:                                                       │
│  • Same Order object, different allowed operations                          │
│  • CREATED state allows cancel, DELIVERED state does not                    │
│  • Each state enforces valid transitions                                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Pattern Components (模式组件)

| Component | Role | Order Processing Example |
|-----------|------|--------------------------|
| **Context** | Maintains current state, delegates | `Order` (maintains `IOrderState`) |
| **State Interface** | Defines state-specific behavior | `IOrderState` |
| **Concrete States** | Implement behavior for each state | `CreatedState`, `DispatchedState`, `DeliveredState` |
| **Transition** | Change from one state to another | `Order.TransitionTo(newState)` |

---

## 🔥 Problem It Solves

### The Anti-Pattern (Without State Pattern)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ❌ ANTI-PATTERN: GIANT SWITCH STATEMENT                 │
├─────────────────────────────────────────────────────────────────────────────┤
│  public class Order                                                         │
│  {                                                                          │
│      public OrderStatus Status { get; set; }                                │
│                                                                             │
│      public void Cancel()                                                   │
│      {                                                                      │
│          switch (Status)                                                    │
│          {                                                                  │
│              case OrderStatus.CREATED:                                      │
│                  // 50 lines: Cancel logic for created orders               │
│                  Status = OrderStatus.CANCELLED;                            │
│                  break;                                                     │
│                                                                             │
│              case OrderStatus.CONFIRMED:                                    │
│                  // 75 lines: Cancel logic for confirmed orders             │
│                  // Need to refund payment                                  │
│                  Status = OrderStatus.CANCELLED;                            │
│                  break;                                                     │
│                                                                             │
│              case OrderStatus.DISPATCHED:                                   │
│                  // 100 lines: Cancel logic for dispatched orders           │
│                  // Need to cancel with carrier                             │
│                  // Need to refund payment                                  │
│                  Status = OrderStatus.CANCELLED;                            │
│                  break;                                                     │
│                                                                             │
│              case OrderStatus.IN_TRANSIT:                                   │
│                  throw new InvalidOperationException("Cannot cancel");      │
│                                                                             │
│              case OrderStatus.DELIVERED:                                    │
│                  throw new InvalidOperationException("Cannot cancel");      │
│                                                                             │
│              // ... more cases for each status                              │
│          }                                                                  │
│      }                                                                      │
│                                                                             │
│      public void Ship()                                                     │
│      {                                                                      │
│          switch (Status)  // Another giant switch!                          │
│          {                                                                  │
│              // ... similar explosion of cases                              │
│          }                                                                  │
│      }                                                                      │
│                                                                             │
│      public void Deliver()                                                  │
│      {                                                                      │
│          switch (Status)  // Yet another giant switch!                      │
│          {                                                                  │
│              // ... and again                                               │
│          }                                                                  │
│      }                                                                      │
│  }                                                                          │
│                                                                             │
│  PROBLEMS:                                                                  │
│  ❌ Order class grows to 1000+ lines                                        │
│  ❌ Adding new status requires editing ALL methods                          │
│  ❌ Easy to forget a case in one of the switches                            │
│  ❌ Business rules for each state scattered across methods                  │
│  ❌ Testing requires testing all combinations                               │
│  ❌ Violates Open/Closed Principle                                          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Real-World Pain Point (真实痛点)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    京东物流订单状态管理演进 (JD Logistics Evolution)           │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  Phase 1 (2010): Simple enum + switch                                       │
│  ────────────────────────────────────                                       │
│  • 5 statuses: CREATED, SHIPPED, DELIVERED, CANCELLED, EXCEPTION            │
│  • Switch statements manageable                                             │
│  • 1 developer could understand entire flow                                 │
│                                                                              │
│  Phase 2 (2015): Growth pain                                                │
│  ────────────────────────────────────                                       │
│  • 20+ statuses: Added warehouse states, carrier states, return states      │
│  • Switch statements became unmanageable                                    │
│  • Bugs: "Order cancelled but carrier not notified"                         │
│  • Each new status = 15 files to modify                                     │
│                                                                              │
│  Phase 3 (2018): State Pattern adoption                                     │
│  ────────────────────────────────────                                       │
│  • Each state in own class                                                  │
│  • Adding new state = adding new class (Open for extension)                 │
│  • Transitions explicit and testable                                        │
│  • 青龙系统 (Qinglong) refactored to State Pattern                          │
│                                                                              │
│  Phase 4 (2023): State + Event Sourcing                                     │
│  ────────────────────────────────────                                       │
│  • All transitions recorded as events                                       │
│  • Complete audit trail                                                     │
│  • Ability to replay order history                                          │
│  • 50+ states managed cleanly                                               │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 🏗️ Pattern Structure

### Class Diagram (类图)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         STATE PATTERN STRUCTURE                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         Order (Context)                              │    │
│  │  ─────────────────────────────────────────────────────────────────  │    │
│  │  - _state: IOrderState                                              │    │
│  │  - Id: Guid                                                         │    │
│  │  - OrderNumber: string                                              │    │
│  │  - Customer: CustomerInfo                                           │    │
│  │                                                                     │    │
│  │  + CurrentStatus: OrderStatus { get => _state.Status }              │    │
│  │                                                                     │    │
│  │  + Confirm()          // Delegates to _state.Confirm(this)          │    │
│  │  + Dispatch()         // Delegates to _state.Dispatch(this)         │    │
│  │  + MarkInTransit()    // Delegates to _state.MarkInTransit(this)    │    │
│  │  + Deliver()          // Delegates to _state.Deliver(this)          │    │
│  │  + Cancel(reason)     // Delegates to _state.Cancel(this, reason)   │    │
│  │                                                                     │    │
│  │  + TransitionTo(newState: IOrderState)  // Internal state change    │    │
│  │    {                                                                │    │
│  │        var oldState = _state;                                       │    │
│  │        _state = newState;                                           │    │
│  │        _state.OnEnter(this, oldState);                              │    │
│  │        RaiseEvent(new OrderStateChanged(oldState, newState));       │    │
│  │    }                                                                │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                    │                                         │
│                                    │ uses                                    │
│                                    ▼                                         │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                 <<interface>> IOrderState                            │    │
│  │  ─────────────────────────────────────────────────────────────────  │    │
│  │  + Status: OrderStatus { get; }                                     │    │
│  │  + CanCancel: bool { get; }                                         │    │
│  │  + CanModify: bool { get; }                                         │    │
│  │                                                                     │    │
│  │  + Confirm(context: Order)                                          │    │
│  │  + Dispatch(context: Order)                                         │    │
│  │  + MarkInTransit(context: Order)                                    │    │
│  │  + Deliver(context: Order)                                          │    │
│  │  + Cancel(context: Order, reason: string)                           │    │
│  │  + OnEnter(context: Order, previousState: IOrderState)              │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                    │                                         │
│         ┌──────────────────────────┼──────────────────────────┐             │
│         │              │           │           │              │             │
│         ▼              ▼           ▼           ▼              ▼             │
│  ┌───────────┐  ┌───────────┐ ┌───────────┐ ┌───────────┐ ┌───────────┐    │
│  │ Created   │  │ Confirmed │ │Dispatched │ │ InTransit │ │ Delivered │    │
│  │  State    │  │   State   │ │   State   │ │   State   │ │   State   │    │
│  │           │  │           │ │           │ │           │ │           │    │
│  │CanCancel  │  │CanCancel  │ │CanCancel  │ │CanCancel  │ │CanCancel  │    │
│  │  = true   │  │  = true   │ │  = true   │ │  = false  │ │  = false  │    │
│  └───────────┘  └───────────┘ └───────────┘ └───────────┘ └───────────┘    │
│                                                                              │
│  Each concrete state:                                                        │
│  • Implements valid operations for that state                               │
│  • Throws InvalidStateTransitionException for invalid operations            │
│  • Knows which states it can transition to                                  │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### State Transition Table (状态转换表)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                         STATE TRANSITION TABLE                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────┬───────────────────────────────────────────────────────┐ │
│  │ Current State  │                    Actions                            │ │
│  │                ├─────────┬─────────┬─────────┬─────────┬───────────────┤ │
│  │                │ Confirm │ Dispatch│ Deliver │ Cancel  │ RequestReturn │ │
│  ├────────────────┼─────────┼─────────┼─────────┼─────────┼───────────────┤ │
│  │ CREATED        │ → CONF  │ ❌      │ ❌      │ → CANC  │ ❌            │ │
│  ├────────────────┼─────────┼─────────┼─────────┼─────────┼───────────────┤ │
│  │ CONFIRMED      │ ❌      │ → DISP  │ ❌      │ → CANC  │ ❌            │ │
│  ├────────────────┼─────────┼─────────┼─────────┼─────────┼───────────────┤ │
│  │ DISPATCHED     │ ❌      │ ❌      │ ❌      │ → CANC* │ ❌            │ │
│  ├────────────────┼─────────┼─────────┼─────────┼─────────┼───────────────┤ │
│  │ IN_TRANSIT     │ ❌      │ ❌      │ → DELIV │ ❌      │ ❌            │ │
│  ├────────────────┼─────────┼─────────┼─────────┼─────────┼───────────────┤ │
│  │ DELIVERED      │ ❌      │ ❌      │ ❌      │ ❌      │ → RET_REQ     │ │
│  ├────────────────┼─────────┼─────────┼─────────┼─────────┼───────────────┤ │
│  │ CANCELLED      │ ❌      │ ❌      │ ❌      │ ❌      │ ❌            │ │
│  └────────────────┴─────────┴─────────┴─────────┴─────────┴───────────────┘ │
│                                                                              │
│  Legend:                                                                     │
│  → STATE = Transitions to that state                                        │
│  ❌ = Throws InvalidStateTransitionException                                │
│  * = Has additional business rules (e.g., must cancel with carrier)         │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 📦 Logistics Application

### Order State Machine (订单状态机)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ORDER STATE MACHINE (订单状态机)                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│                              ┌─────────┐                                    │
│                              │ CREATED │                                    │
│                              │ 已创建   │                                    │
│                              └────┬────┘                                    │
│                     payment       │      cancel                              │
│                     received      │      requested                           │
│                      ┌────────────┴────────────┐                            │
│                      ▼                         ▼                             │
│                ┌───────────┐             ┌───────────┐                      │
│                │ CONFIRMED │             │ CANCELLED │                      │
│                │ 已确认     │             │ 已取消     │                      │
│                └─────┬─────┘             └───────────┘                      │
│                      │                         ▲                             │
│                      │ carrier                 │                             │
│                      │ booked                  │ cancel                      │
│                      ▼                         │ (before pickup)             │
│                ┌────────────┐                  │                             │
│                │ DISPATCHED ├──────────────────┘                             │
│                │ 已发货      │                                                │
│                └─────┬──────┘                                                │
│                      │ carrier                                               │
│                      │ pickup                                                │
│                      ▼                                                       │
│                ┌────────────┐                                                │
│                │ IN_TRANSIT │                                                │
│                │ 运输中      │                                                │
│                └─────┬──────┘                                                │
│                      │ out for                                               │
│                      │ delivery                                              │
│                      ▼                                                       │
│           ┌──────────────────────┐                                          │
│           │  OUT_FOR_DELIVERY    │                                          │
│           │  派送中               │                                          │
│           └──────────┬───────────┘                                          │
│             success  │     failure                                           │
│             ┌────────┴────────┐                                             │
│             ▼                 ▼                                              │
│       ┌───────────┐    ┌────────────────┐                                   │
│       │ DELIVERED │    │ FAILED_DELIVERY│◄──┐                               │
│       │ 已签收     │    │ 派送失败        │   │ retry                         │
│       └─────┬─────┘    └───────┬────────┘   │                               │
│             │                  │            │                                │
│             │                  └────────────┘                                │
│             │                                                                │
│             │ customer                                                       │
│             │ requests return                                                │
│             ▼                                                                │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                    REVERSE LOGISTICS FLOW (逆向物流)                 │    │
│  │                                                                     │    │
│  │   ┌─────────────────┐     ┌─────────────────┐    ┌──────────────┐  │    │
│  │   │ RETURN_REQUESTED│ ──▶ │ RETURN_IN_TRANSIT│ ──▶│RETURN_RECEIVED│  │    │
│  │   │ 退货申请         │     │ 退货运输中        │    │ 退货已签收    │  │    │
│  │   └─────────────────┘     └─────────────────┘    └──────┬───────┘  │    │
│  │                                                         │          │    │
│  │                                                         ▼          │    │
│  │                                                  ┌──────────────┐  │    │
│  │                                                  │   REFUNDED   │  │    │
│  │                                                  │   已退款      │  │    │
│  │                                                  └──────────────┘  │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Concrete State Implementation Example

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    CONCRETE STATE: CreatedState                              │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  public class CreatedState : IOrderState                                     │
│  {                                                                           │
│      // Properties                                                           │
│      public OrderStatus Status => OrderStatus.CREATED;                       │
│      public bool CanCancel => true;    // ✅ Can cancel newly created order  │
│      public bool CanModify => true;    // ✅ Can modify address, items       │
│                                                                              │
│      // Allowed transitions                                                  │
│      public void Confirm(Order context)                                      │
│      {                                                                       │
│          // Business logic for confirmation                                  │
│          if (!context.HasValidPayment)                                       │
│              throw new OrderException("Payment required");                   │
│                                                                              │
│          context.ConfirmedAt = DateTime.UtcNow;                              │
│          context.TransitionTo(new ConfirmedState());                         │
│      }                                                                       │
│                                                                              │
│      public void Cancel(Order context, string reason)                        │
│      {                                                                       │
│          // Simple cancellation - no carrier to notify yet                   │
│          context.CancelledAt = DateTime.UtcNow;                              │
│          context.CancellationReason = reason;                                │
│          context.TransitionTo(new CancelledState());                         │
│      }                                                                       │
│                                                                              │
│      // Invalid transitions - throw exceptions                               │
│      public void Dispatch(Order context)                                     │
│          => throw new InvalidStateTransitionException(                       │
│              Status, OrderStatus.DISPATCHED, "Must confirm first");          │
│                                                                              │
│      public void MarkInTransit(Order context)                                │
│          => throw new InvalidStateTransitionException(                       │
│              Status, OrderStatus.IN_TRANSIT, "Must dispatch first");         │
│                                                                              │
│      public void Deliver(Order context)                                      │
│          => throw new InvalidStateTransitionException(                       │
│              Status, OrderStatus.DELIVERED, "Must be in transit first");     │
│                                                                              │
│      // Lifecycle hook                                                       │
│      public void OnEnter(Order context, IOrderState previousState)           │
│      {                                                                       │
│          // Called when entering this state                                  │
│          context.AddDomainEvent(new OrderCreatedEvent(context));             │
│      }                                                                       │
│  }                                                                           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    CONCRETE STATE: InTransitState                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  public class InTransitState : IOrderState                                   │
│  {                                                                           │
│      public OrderStatus Status => OrderStatus.IN_TRANSIT;                    │
│      public bool CanCancel => false;   // ❌ Too late to cancel              │
│      public bool CanModify => false;   // ❌ Package already moving          │
│                                                                              │
│      // Allowed transitions                                                  │
│      public void MarkOutForDelivery(Order context)                           │
│      {                                                                       │
│          if (context.DriverId == null)                                       │
│              throw new OrderException("Driver must be assigned");            │
│                                                                              │
│          context.OutForDeliveryAt = DateTime.UtcNow;                         │
│          context.TransitionTo(new OutForDeliveryState());                    │
│      }                                                                       │
│                                                                              │
│      public void MarkException(Order context, ExceptionType type)            │
│      {                                                                       │
│          context.ExceptionType = type;                                       │
│          context.ExceptionAt = DateTime.UtcNow;                              │
│          context.TransitionTo(new ExceptionState());                         │
│      }                                                                       │
│                                                                              │
│      // Invalid transitions                                                  │
│      public void Cancel(Order context, string reason)                        │
│          => throw new InvalidStateTransitionException(                       │
│              Status, OrderStatus.CANCELLED,                                  │
│              "Cannot cancel order in transit. Contact carrier.");            │
│                                                                              │
│      public void Confirm(Order context)                                      │
│          => throw new InvalidStateTransitionException(                       │
│              Status, OrderStatus.CONFIRMED, "Already past confirmation");    │
│                                                                              │
│      // ... other invalid transitions                                        │
│  }                                                                           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## ⚖️ State vs Strategy Pattern

### Key Differences (关键区别)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    STATE vs STRATEGY PATTERN COMPARISON                      │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  ┌────────────────────────────────┬────────────────────────────────┐        │
│  │        STATE PATTERN           │       STRATEGY PATTERN         │        │
│  │        状态模式                 │        策略模式                │        │
│  ├────────────────────────────────┼────────────────────────────────┤        │
│  │                                │                                │        │
│  │  PURPOSE:                      │  PURPOSE:                      │        │
│  │  Change behavior based on      │  Choose algorithm at runtime   │        │
│  │  internal state                │  based on external factors     │        │
│  │                                │                                │        │
│  │  根据内部状态改变行为            │  根据外部因素选择算法            │        │
│  │                                │                                │        │
│  ├────────────────────────────────┼────────────────────────────────┤        │
│  │                                │                                │        │
│  │  WHO CHANGES:                  │  WHO CHANGES:                  │        │
│  │  State changes ITSELF          │  Client changes strategy       │        │
│  │  (internal transitions)        │  (external selection)          │        │
│  │                                │                                │        │
│  │  状态自己改变自己                │  客户端选择策略                 │        │
│  │                                │                                │        │
│  ├────────────────────────────────┼────────────────────────────────┤        │
│  │                                │                                │        │
│  │  STATE AWARENESS:              │  STATE AWARENESS:              │        │
│  │  States know about each other  │  Strategies independent        │        │
│  │  (for transitions)             │  (don't know each other)       │        │
│  │                                │                                │        │
│  │  状态之间相互了解                │  策略之间相互独立               │        │
│  │                                │                                │        │
│  ├────────────────────────────────┼────────────────────────────────┤        │
│  │                                │                                │        │
│  │  EXAMPLE:                      │  EXAMPLE:                      │        │
│  │  Order lifecycle               │  Route calculation algorithm   │        │
│  │  (CREATED → CONFIRMED → ...)   │  (FastestRoute, CheapestRoute) │        │
│  │                                │                                │        │
│  └────────────────────────────────┴────────────────────────────────┘        │
│                                                                              │
│  VISUAL COMPARISON:                                                          │
│                                                                              │
│  STATE PATTERN:                    STRATEGY PATTERN:                         │
│  ┌───┐ → ┌───┐ → ┌───┐            ┌───────────────────┐                     │
│  │ A │   │ B │   │ C │            │                   │                     │
│  └───┘   └───┘   └───┘            │     Context       │                     │
│    │       │       │              │                   │                     │
│  Object progresses through        │   uses ONE of:    │                     │
│  states over its lifetime         │                   │                     │
│                                   └───────┬───────────┘                     │
│                                     ┌─────┼─────┐                           │
│                                     ▼     ▼     ▼                           │
│                                   ┌───┐ ┌───┐ ┌───┐                         │
│                                   │ A │ │ B │ │ C │                         │
│                                   └───┘ └───┘ └───┘                         │
│                                   Client picks one                          │
│                                   strategy to use                           │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

### When to Use Each (何时使用)

| Use State Pattern When | Use Strategy Pattern When |
|------------------------|---------------------------|
| Object has lifecycle with distinct states | Need interchangeable algorithms |
| Behavior depends on current state | Client should choose algorithm |
| Valid operations differ by state | Algorithms are independent |
| State transitions follow rules | No notion of "current state" |
| **Example**: Order processing | **Example**: Routing algorithms |

### Can They Coexist? (能否共存?)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    YES! STATE + STRATEGY COMBINED                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  In Order Processing, we use BOTH:                                          │
│                                                                              │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         Order (Context)                              │    │
│  │  ─────────────────────────────────────────────────────────────────  │    │
│  │                                                                     │    │
│  │  - _state: IOrderState             ← STATE PATTERN                  │    │
│  │  - _dispatchStrategy: IDispatchStrategy  ← STRATEGY PATTERN         │    │
│  │                                                                     │    │
│  │  + Dispatch()                                                       │    │
│  │    {                                                                │    │
│  │        _state.Dispatch(this);      // State validates transition    │    │
│  │        // Inside ConfirmedState.Dispatch():                         │    │
│  │        var carrier = _dispatchStrategy.SelectCarrier(this);         │    │
│  │        // Strategy selects carrier algorithm                        │    │
│  │    }                                                                │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                              │
│  The patterns work at different levels:                                      │
│  • State Pattern: Controls WHAT operations are valid                        │
│  • Strategy Pattern: Controls HOW operations are performed                  │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## 🔧 Implementation Variations

### Variation 1: State as Singleton (状态单例)

```
Use Case: States are stateless (no instance data)
优点: Memory efficient
缺点: Cannot store state-specific data

public class CreatedState : IOrderState
{
    // Singleton instance
    public static readonly CreatedState Instance = new();
    private CreatedState() { }  // Private constructor
    
    // All methods work with context data only
}

// Usage:
order.TransitionTo(CreatedState.Instance);
```

### Variation 2: State with Instance Data (带实例数据的状态)

```
Use Case: Each state needs to store data
优点: State can hold metadata
缺点: More memory per order

public class ReturnInTransitState : IOrderState
{
    public string ReturnTrackingNumber { get; }
    public string ReturnCarrierCode { get; }
    public DateTime PickupDate { get; }
    
    public ReturnInTransitState(string trackingNo, string carrier, DateTime pickup)
    {
        ReturnTrackingNumber = trackingNo;
        ReturnCarrierCode = carrier;
        PickupDate = pickup;
    }
}

// Usage:
order.TransitionTo(new ReturnInTransitState("SF123", "SF", DateTime.Today));
```

### Variation 3: State Machine Library (状态机库)

```
Use Case: Complex state machines with many transitions
Tools: Stateless, Automatonymous

Using Stateless library:
─────────────────────────────────────────────

var orderMachine = new StateMachine<Order, OrderStatus, OrderTrigger>(
    () => order.Status,
    s => order.Status = s);

orderMachine.Configure(OrderStatus.CREATED)
    .Permit(OrderTrigger.PaymentReceived, OrderStatus.CONFIRMED)
    .Permit(OrderTrigger.CustomerCancels, OrderStatus.CANCELLED);

orderMachine.Configure(OrderStatus.CONFIRMED)
    .Permit(OrderTrigger.CarrierBooked, OrderStatus.DISPATCHED)
    .Permit(OrderTrigger.CustomerCancels, OrderStatus.CANCELLED)
    .OnEntry(() => NotifyCustomer(order));

orderMachine.Configure(OrderStatus.IN_TRANSIT)
    .Ignore(OrderTrigger.CustomerCancels);  // Cannot cancel

// Fire trigger
orderMachine.Fire(OrderTrigger.PaymentReceived);
```

### Variation 4: Event-Sourced State (事件溯源状态)

```
Use Case: Need complete audit trail
Used by: 京东青龙系统, SF Express

public class Order : AggregateRoot
{
    private IOrderState _state = new CreatedState();
    
    public void Apply(OrderConfirmedEvent e)
    {
        _state = new ConfirmedState();
        ConfirmedAt = e.OccurredAt;
    }
    
    public void Apply(OrderDispatchedEvent e)
    {
        _state = new DispatchedState();
        TrackingNumber = e.TrackingNumber;
        CarrierId = e.CarrierId;
    }
    
    // Rebuild state by replaying events
    public static Order FromHistory(IEnumerable<DomainEvent> events)
    {
        var order = new Order();
        foreach (var e in events)
            order.Apply((dynamic)e);
        return order;
    }
}
```

---

## ⚠️ Anti-Patterns to Avoid

### Anti-Pattern 1: God State (上帝状态)

```
❌ BAD: One state that handles everything

public class AllInOneState : IOrderState
{
    public void Handle(Order order, string action)
    {
        switch (action)
        {
            case "confirm": // ...
            case "dispatch": // ...
            case "deliver": // ...
            // Back to switch statements!
        }
    }
}

✅ GOOD: Separate state class per state
```

### Anti-Pattern 2: Leaking State Logic (状态逻辑泄漏)

```
❌ BAD: Context checks state before delegating

public class Order
{
    public void Cancel(string reason)
    {
        // DON'T DO THIS - logic should be in state
        if (_state.Status == OrderStatus.IN_TRANSIT)
            throw new Exception("Cannot cancel");
            
        _state.Cancel(this, reason);
    }
}

✅ GOOD: Let state handle the logic

public class Order
{
    public void Cancel(string reason)
    {
        _state.Cancel(this, reason);  // State decides
    }
}
```

### Anti-Pattern 3: Circular Dependencies (循环依赖)

```
❌ BAD: States creating each other directly

public class ConfirmedState : IOrderState
{
    public void Dispatch(Order context)
    {
        // Direct coupling to concrete state
        context.TransitionTo(new DispatchedState());
    }
}

✅ GOOD: Use factory or DI

public class ConfirmedState : IOrderState
{
    private readonly IOrderStateFactory _factory;
    
    public void Dispatch(Order context)
    {
        context.TransitionTo(_factory.Create(OrderStatus.DISPATCHED));
    }
}
```

### Anti-Pattern 4: Missing Transition Validation (缺少转换验证)

```
❌ BAD: Allow any transition

public class ConfirmedState : IOrderState
{
    public void Deliver(Order context)
    {
        // Skips DISPATCHED and IN_TRANSIT!
        context.TransitionTo(new DeliveredState());
    }
}

✅ GOOD: Enforce valid transitions only

public class ConfirmedState : IOrderState
{
    public void Deliver(Order context)
    {
        throw new InvalidStateTransitionException(
            Status, OrderStatus.DELIVERED,
            "Must dispatch and ship before delivery");
    }
}
```

---

## 🚀 Advanced Topics

### Topic 1: Hierarchical State Machines (层次状态机)

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    HIERARCHICAL STATE MACHINE                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Use Case: States have sub-states (like 京东物流仓配一体化)                   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         IN_WAREHOUSE (大状态)                        │    │
│  │  ┌───────────────────────────────────────────────────────────────┐  │    │
│  │  │                                                               │  │    │
│  │  │   ┌─────────┐    ┌─────────┐    ┌─────────┐    ┌─────────┐    │  │    │
│  │  │   │ALLOCATED│ ─▶ │ PICKING │ ─▶ │ PACKED  │ ─▶ │HANDED_ │   │  │    │
│  │  │   │ 已分配   │    │ 拣货中   │    │ 已打包   │    │  OFF    │   │  │    │
│  │  │   └─────────┘    └─────────┘    └─────────┘    │ 已交接   │    │  │    │
│  │  │                                                └─────────┘    │  │    │
│  │  │                                                               │  │    │
│  │  └───────────────────────────────────────────────────────────────┘  │    │
│  └────────────────────────────────────────────────────────────────┬────┘    │
│                                                                   │         │
│                                                                   ▼         │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                         IN_TRANSIT (大状态)                          │    │
│  │  ┌───────────────────────────────────────────────────────────────┐  │    │
│  │  │   ┌─────────┐    ┌─────────┐    ┌─────────┐                   │  │    │
│  │  │   │ HUB_1   │ ─> │ HUB_2   │ ─> │  LAST   │                   │  │    │
│  │  │   │ 转运中心1│    │ 转运中心2│   │  MILE   │                   │  │     │
│  │  │   └─────────┘    └─────────┘    │ 末端派送 │                   │  │    │
│  │  │                                 └─────────┘                   │  │     │
│  │  └───────────────────────────────────────────────────────────────┘  │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                             │
│  Implementation: Composite State Pattern                                    │
│  • Parent state delegates to active sub-state                               │
│  • Sub-states can transition within parent                                  │
│  • Parent handles transitions to other parent states                        │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### Topic 2: Concurrent States (并发状态)

```
Use Case: Order has multiple independent aspects
Example: Payment status + Fulfillment status

┌─────────────────────────────────────────────────────────────────┐
│                    Order                                        │
│  ┌──────────────────────┐    ┌──────────────────────┐           │
│  │   Payment Region     │    │  Fulfillment Region  │           │
│  │                      │    │                      │           │
│  │  PENDING             │    │  CREATED             │           │
│  │    │                 │    │    │                 │           │
│  │    ▼                 │    │    ▼                 │           │
│  │  PAID                │    │  DISPATCHED          │           │
│  │    │                 │    │    │                 │           │
│  │    ▼                 │    │    ▼                 │           │
│  │  REFUNDED            │    │  DELIVERED           │           │
│  │                      │    │                      │           │
│  └──────────────────────┘    └──────────────────────┘           │
│                                                                 │
│  States operate independently:                                  │
│  • Can be PAID + DISPATCHED                                     │
│  • Can be REFUNDED + DELIVERED (customer returned)              │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

### Topic 3: State Persistence (状态持久化)

```
Options for saving state to database:
─────────────────────────────────────────────

Option 1: Enum column (simple)
  Orders table: Status INT NOT NULL

Option 2: State table (flexible)
  OrderStates table: OrderId, StateType, EnteredAt, Data (JSON)

Option 3: Event sourcing (complete history)
  OrderEvents table: OrderId, EventType, EventData, OccurredAt
  
Recommendation:
• Simple workflows: Enum column
• Need audit: Event sourcing
• Complex sub-states: State table with JSON
```

---

## 🇨🇳 Chinese Tech References

### Industry Examples (行业案例)

| Company | State Machine Complexity | Key Learning |
|---------|--------------------------|--------------|
| 京东物流 | 50+ states (仓配一体化) | Hierarchical states for warehouse + delivery |
| 顺丰速运 | 30+ states (标准化) | Time-bound states with SLA |
| 中通快递 | 25+ states (简洁化) | State + SubStatus for flexibility |
| 菜鸟网络 | 40+ states (协同) | Cross-company state synchronization |

### Chinese Tech Community Resources

| Platform | Search Keywords | Content Type |
|----------|-----------------|--------------|
| CSDN | `订单状态机 设计模式` | Implementation tutorials |
| 掘金 | `状态模式 物流系统` | Real-world case studies |
| 知乎 | `电商订单 状态管理` | Architecture discussions |
| Gitee | `状态机 DDD` | Open source examples |

### Recommended Articles (推荐文章)

```
1. "京东物流订单状态机演进之路" - 京东技术
   内容：从简单枚举到50+状态的演进过程

2. "顺丰时效承诺背后的状态管理" - 顺丰科技
   内容：如何用状态模式实现时效SLA

3. "中通快递状态码标准化实践" - 中通技术
   内容：状态+子状态的设计思路

4. "菜鸟网络多承运商状态同步" - 阿里技术
   内容：跨公司状态映射与同步
```

---

## 📝 Self-Assessment

### Knowledge Check (知识检查)

**Q1**: What problem does State Pattern solve?
<details>
<summary>Answer</summary>
Eliminates large switch/if-else statements that check object state before executing behavior. Instead, behavior is encapsulated in state classes.
</details>

**Q2**: Who is responsible for state transitions - the context or the state?
<details>
<summary>Answer</summary>
The state classes initiate transitions (they know what states they can transition to), but the context executes the actual state change via `TransitionTo()`.
</details>

**Q3**: What's the key difference between State and Strategy patterns?
<details>
<summary>Answer</summary>
- State: Object behavior changes as internal state changes (states know each other)
- Strategy: Client chooses algorithm (strategies are independent)
</details>

**Q4**: When would you use a state machine library vs. manual implementation?
<details>
<summary>Answer</summary>
- Manual: Simple state machines (< 10 states), need full control
- Library: Complex state machines, need visualization, configuration-driven
</details>

**Q5**: How do you persist state in a database?
<details>
<summary>Answer</summary>
- Simple: Enum/int column for status
- Audit needed: Event sourcing (store all transitions)
- Complex sub-states: JSON column or separate state table
</details>

### Coding Exercise (编码练习)

```
Exercise: Implement a simplified Order state machine

Requirements:
1. States: CREATED, CONFIRMED, SHIPPED, DELIVERED, CANCELLED
2. Transitions:
   - CREATED → CONFIRMED (on payment)
   - CREATED → CANCELLED (customer cancels)
   - CONFIRMED → SHIPPED (on dispatch)
   - CONFIRMED → CANCELLED (customer cancels)
   - SHIPPED → DELIVERED (on delivery)
3. SHIPPED and DELIVERED cannot be cancelled

Bonus:
- Add OnEnter/OnExit hooks
- Add transition guards (e.g., cannot confirm without payment)
- Add domain events for each transition

Time: 60 minutes
```

### Architecture Discussion (架构讨论)

```
Scenario: You're designing an order system for a company that:
- Has 15 different order states
- Needs to support returns (reverse logistics)
- Must integrate with 5 different carriers (each with their own status codes)
- Requires complete audit trail
- Handles 100,000 orders/day

Questions:
1. Would you use State Pattern? Why or why not?
2. How would you map carrier statuses to your internal states?
3. How would you persist state changes for audit?
4. How would you handle the complexity of 15 states?

Discuss with your team or write your thoughts.
```

---

## 🔗 Related Documents

- **Domain Spec**: [04-ORDER-PROCESSING.md](../core-domains/04-ORDER-PROCESSING.md)
- **Related Pattern**: [STRATEGY-PATTERN.md](STRATEGY-PATTERN.md) - Often confused with State
- **Related Pattern**: [OBSERVER-PATTERN.md](OBSERVER-PATTERN.md) - For state change notifications
- **CQRS Pattern**: [CQRS-PATTERN.md](CQRS-PATTERN.md) - Often used together

---